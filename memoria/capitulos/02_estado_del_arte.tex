...
\begin{description}
  \item[Electroencefalografía (EEG).] No invasiva, de bajo coste y ampliamente disponible. Registra la actividad sumada de grandes poblaciones neuronales a través del cráneo, lo que impone una resolución espacial de centímetros y una relación señal-ruido baja. Es adecuada para paradigmas de selección discreta, pero la decodificación de habla continua a partir de EEG sigue siendo objeto de debate metodológico, en particular en lo relativo a la independencia entre particiones y a la elección de métricas que premian la similitud semántica frente a la recuperación literal del estímulo.
% TODO (Kiril): esta frase necesita una cita o hay que suavizarla mas. Es una
% afirmacion contestable y ahora mismo va a pelo. Busca alguna revision critica
% del campo; si no encuentras ninguna solida, borra la subordinada entera.
...
A lo largo de esta sección se hace referencia repetidamente a los dos \textit{benchmarks} públicos del campo, Brain-to-Text~'24 y Brain-to-Text~'25, y a sus respectivos sistemas de referencia, por ser el punto de comparación común de la literatura reciente. Ambos se describen en detalle en la \cref{sec:sota-benchmarks}; basta por ahora con saber que son conjuntos de datos abiertos con particiones fijas y clasificación pública, y que el sistema de referencia de los dos es la red recurrente descrita a continuación.
...
La literatura emplea el modelo de lenguaje en dos regímenes cualitativamente distintos, el reordenamiento de una lista de hipótesis ya generada y la fusión superficial durante la propia búsqueda. La distinción es central en esta memoria y se formaliza en la \cref{sec:oracle}.
...
Estas técnicas comparten una condición de aplicabilidad que condiciona el diseño experimental de esta memoria: presuponen un modelo base congelado, ya que su beneficio no procede de añadir capacidad sino de restringir la adaptación. El argumento formal se desarrolla en la \cref{sec:lora-teoria}.
