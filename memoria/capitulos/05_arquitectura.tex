...
La solución adoptada sigue la práctica estándar del campo, introducida en \cite{willett2021} y presente en la mayoría de los sistemas posteriores (\cref{sec:sota-anatomia}): aprender una transformación de entrada propia de cada día. La aportación de este trabajo no está en la idea, sino en su instanciación concreta y en su evaluación experimental, incluida la cuantificación de su coste y de su comportamiento ante días no vistos.
...
donde $s$ es el índice de la sesión a la que pertenece el ensayo y $\mathrm{softsign}(u) = u/(1+|u|)$ es una no linealidad acotada en $(-1,1)$, de saturación más suave que la tangente hiperbólica, que impide que una matriz de sesión mal condicionada propague valores extremos al adaptador. Cada matriz $\mathbf{W}_s$ se inicializa a la identidad y cada vector $\mathbf{b}_s$ a cero, de modo que al comenzar el entrenamiento la capa se limita a aplicar la función \textit{softsign} y el sistema parte de un comportamiento neutro.
...
\begin{table}[H]
  ...
  \begin{tabular}{l r p{5.3cm} l}
    ...
    Subpalabra propia    & 256 & \textit{SentencePiece} sobre las transcripciones de entrenamiento & CER/WER \\
    Palabra              & Variable & Vocabulario cerrado de entrenamiento & WER \\
    \bottomrule
  \end{tabular}
\end{table}

Dos observaciones sobre este componente. La primera es que esta capa se entrena desde cero en todos los casos, de modo que un vocabulario de 50.002 clases reparte poco más de ocho mil frases entre ellas y deja a la inmensa mayoría sin ejemplos, mientras que con 29 clases de carácter cada una recibe miles. Esta asimetría motivó la expectativa inicial de que los vocabularios grandes resultasen inviables; el \cref{cap:resultados} muestra que la expectativa era incorrecta y por qué.
...
El sistema permite escalar la tasa de aprendizaje del codificador por un factor, manteniendo la tasa base para el adaptador, la cabeza CTC y los adaptadores de rango bajo. La separación se realiza por nombre de parámetro al construir el optimizador.

La motivación inicial fue la habitual: aplicar una misma tasa a componentes preentrenados y a componentes inicializados desde cero es cuestionable, porque los primeros parten de un óptimo y los segundos del ruido. Sin embargo, el mecanismo resultó ser necesario también \textbf{con el codificador inicializado aleatoriamente}, donde ese argumento no aplica: con tasa unificada el entrenamiento diverge. La tasa diferenciada cumple aquí, por tanto, una segunda función independiente de la procedencia de los pesos, la de estabilizar el acoplamiento entre un adaptador que aprende deprisa y un codificador profundo que aún no ha organizado sus representaciones. Ambas funciones se distinguen explícitamente al interpretar los resultados.
