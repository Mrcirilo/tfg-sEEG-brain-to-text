...
Para este trabajo, la consecuencia práctica es determinante: al contemplar la arquitectura una rama CTC sobre la salida del codificador, \textbf{el modelo puede operarse en régimen puramente CTC} fijando $\lambda = 1$ y prescindiendo por completo del decodificador autorregresivo. Lo que se reutiliza es la vía arquitectónica, no los pesos de la cabeza CTC preentrenada, que se sustituyen en cualquier caso al cambiar el vocabulario de salida. Esto elimina la dependencia del vocabulario de subpalabras preentrenado y del comportamiento generativo del decodificador, dos elementos que en un régimen de datos escasos resultan más un lastre que una ventaja.
