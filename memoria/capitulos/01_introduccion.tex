...
A esta población se suman los supervivientes de ictus de tronco y otras lesiones neurológicas que cursan con anartria. Para todas ellas, la pérdida del habla no supone únicamente una limitación funcional: supone la pérdida de la autonomía en la relación con los demás, y aparece de forma sistemática entre las prioridades más valoradas por los propios pacientes cuando se les pregunta qué función desearían recuperar.
...
Los sistemas que encabezan actualmente el \textit{benchmark} combinan decenas de modelos entrenados con semillas distintas y emplean modelos de lenguaje de gran tamaño para la fusión de sus salidas; incluso el modelo de lenguaje $n$-grama oficial de la competición requiere una cantidad de memoria muy superior a la disponible en un entorno de trabajo estándar. Este trabajo se desarrolla íntegramente sobre datos públicos, bibliotecas de código abierto y una única GPU accesible mediante un servicio en la nube de bajo coste, y documenta de forma explícita el presupuesto de cómputo de cada experimento. Mostrar qué parte del problema sigue siendo abordable en esas condiciones tiene valor por sí mismo, tanto para grupos de investigación con recursos limitados como para el propio criterio de reproducibilidad de los resultados publicados.
...
\begin{enumerate}[label=\textbf{OE\arabic*.}, leftmargin=*, itemsep=0.4em]
  ...
  \item \textbf{Diseñar el módulo de adaptación de modalidad} que sustituya al \textit{frontend} acústico del modelo, de forma que las 512 características neuronales por ventana temporal se proyecten al espacio de entrada esperado por el codificador, e incorporar un mecanismo de normalización por sesión que compense la deriva de la señal entre días de registro.
...
El resto de la memoria se organiza como sigue. El \cref{cap:estado-del-arte} revisa el estado del arte en neuroprótesis del habla y en la adaptación de modelos de habla preentrenados, y sitúa la aportación de este trabajo respecto a ambas líneas. El \cref{cap:datos} describe el conjunto de datos empleado, su proceso de adquisición y sus particularidades, y lo compara con los conjuntos de datos alternativos disponibles en el campo. El \cref{cap:metodologia} detalla la arquitectura del sistema propuesto, la función de pérdida empleada y el procedimiento de entrenamiento y evaluación. El \cref{cap:experimentos} presenta el diseño experimental, organizado en bloques que responden a cada una de las preguntas de investigación planteadas. El \cref{cap:resultados} recoge y discute los resultados obtenidos. Finalmente, el \cref{cap:conclusiones} resume las conclusiones del trabajo y apunta las líneas de continuación que se consideran más prometedoras.
